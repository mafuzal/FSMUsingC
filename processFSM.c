...
void substring_occurence_counter(state_t* from, state_t* to,
								 char* input_buff,
...
{
	fsm_t* fsm = create_new_fsm("FSM Subtring Counter");
...
fsm_t* email_validator_fsm()
{
	...
	create_and_insert_new_tt_entry(&state_q6->state_trans_table,
								   transition_key, transition_keysize, 0, state_F);
	create_and_insert_new_tt_entry_wild_card(state_q6, DEAD_STATE, 0);

	/*q5 */
	state_t* state_q5 = create_new_state("q5", FSM_FALSE);
	tt_entry = create_and_insert_new_tt_entry(&state_q5->state_trans_table, 0, 0, 0, state_q5);
	...
	create_and_insert_new_tt_entry(&state_q5->state_trans_table,
								   transition_key, transition_keysize, 0, state_q6);
	...
	tt_entry = create_and_insert_new_tt_entry(&state_q3->state_trans_table, 0, 0, 0, state_q4);
	register_input_matching_tt_entry_cb(tt_entry, match_any_0_9_match_fn);
	register_input_matching_tt_entry_cb(tt_entry, match_any_a_z_match_fn);
	register_input_matching_tt_entry_cb(tt_entry, match_any_A_Z_match_fn);
	...
	tt_entry = create_and_insert_new_tt_entry(&state_q2->state_trans_table, 0, 0, 0, state_q3);
	register_input_matching_tt_entry_cb(tt_entry, match_any_0_9_match_fn);
	register_input_matching_tt_entry_cb(tt_entry, match_any_a_z_match_fn);
	register_input_matching_tt_entry_cb(tt_entry, match_any_A_Z_match_fn);
	...
	tt_entry = create_and_insert_new_tt_entry(&state_q1->state_trans_table, 0, 0, 0, state_q2);
	register_input_matching_tt_entry_cb(tt_entry, match_any_0_9_match_fn);
	register_input_matching_tt_entry_cb(tt_entry, match_any_a_z_match_fn);
	register_input_matching_tt_entry_cb(tt_entry, match_any_A_Z_match_fn);
	...
	tt_entry = create_and_insert_new_tt_entry(&state_q1->state_trans_table, 0, 0, 0, state_q1);
	register_input_matching_tt_entry_cb(tt_entry, match_any_0_9_match_fn);
	register_input_matching_tt_entry_cb(tt_entry, match_any_a_z_match_fn);
	register_input_matching_tt_entry_cb(tt_entry, match_any_A_Z_match_fn);
...
void convert_binary_to_hex(state_t* from, state_t* to,
							char* input_buff,
							unsigned int input_buff_size,
							fsm_output_buff_t* fsm_output_buff)
{
...
fsm_t* fsm_binary_to_hex()
{
	...
	create_and_insert_new_tt_entry(&q4->state_trans_table, "0", 1, 0, q1);
	create_and_insert_new_tt_entry(&q4->state_trans_table, "1", 1, 0, q1);

	/*q3*/
	state_t* q3 = create_new_state("q3", FSM_FALSE);
	create_and_insert_new_tt_entry(&q3->state_trans_table, "0", 1, convert_binary_to_hex, q4);
	create_and_insert_new_tt_entry(&q3->state_trans_table, "1", 1, convert_binary_to_hex, q4);

	/*q2*/
	state_t* q2 = create_new_state("q2", FSM_FALSE);
	create_and_insert_new_tt_entry(&q3->state_trans_table, "0", 1, 0, q3);
	create_and_insert_new_tt_entry(&q3->state_trans_table, "1", 1, 0, q3);

	/*q1*/
	state_t* q1 = create_new_state("q1", FSM_FALSE);
	create_and_insert_new_tt_entry(&q3->state_trans_table, "0", 1, 0, q2);
	create_and_insert_new_tt_entry(&q3->state_trans_table, "1", 1, 0, q2);

	/*q0*/
	state_t* q0 = create_new_state("q0", FSM_FALSE);
	set_fsm_initial_state(fsm, q0);
	create_and_insert_new_tt_entry(&q0->state_trans_table, "0", 1, 0, q1);
	create_and_insert_new_tt_entry(&q0->state_trans_table, "1", 1, 0, q1);

	return fsm;
}

static fsm_bool_t match_any_0_4_match_fn(char* data1,
										unsigned int size,
										char* data2,
										unsigned int user_data_len,
										unsigned int* length_read)
...
static fsm_bool_t match_any_0_5_match_fn(char* data1,
										unsigned int size,
										char* data2,
										unsigned int user_data_len,
										unsigned int* length_read)
...
static fsm_t* ip_address_octet_validator()
{
	...
	transition_key = '5';
	tt_entry = create_and_insert_new_tt_entry(&state_q3->state_trans_table, &transition_key, 1, 0, state_q5);
	create_and_insert_new_tt_entry_wild_card(state_q3, DEAD_STATE, 0);

	/*q2 */
	state_t* state_q2 = create_new_state("q2", FSM_FALSE);
	tt_entry = create_and_insert_new_tt_entry(&state_q2->state_trans_table, 0, 0, 0, state_q6);
	register_input_matching_tt_entry_cb(tt_entry, match_any_0_9_match_fn);
	create_and_insert_new_tt_entry_wild_card(state_q2, DEAD_STATE, 0);
	...
	transition_key = '0';
	tt_entry = create_and_insert_new_tt_entry(&state_q0->state_trans_table, &transition_key, 1, 0, state_q1);
	transition_key = '1';
	tt_entry = create_and_insert_new_tt_entry(&state_q0->state_trans_table, &transition_key, 1, 0, state_q1);
	transition_key = '2';
	tt_entry = create_and_insert_new_tt_entry(&state_q0->state_trans_table, &transition_key, 1, 0, state_q1);
