...
extern fsm_bool_t match_any_character_match_fn(char* data1, unsigned int size,
		char* data2, unsigned int user_data_len,
		unsigned int* length_read);


static fsm_bool_t fsm_default_input_matching_fn(char* transition_key, unsigned int size,
		char* user_data, unsigned int user_data_len,
		unsigned int* length_read)
{
...
void fsm_echo_output_fn(state_t* from, state_t* to, char* input_buff,
		unsigned int input_buff_size,
		fsm_output_buff_t* fsm_output_buff)
{
	memccpy(fsm_output_buff->output_buffer + fsm_output_buff->curr_pos,
