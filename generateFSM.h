...
typedef unsigned int (*input_fn)(  /*Returns the size of input buffer read*/
		...
		unsigned int,                  /*Starting posiiton to read the buffer*/
		char*,                         /*Data Read*/
		unsigned int*,                 /*Size of Data Read*/
		unsigned int                   /*Max len of read out of buffer*/
);
...
static inline tt_entry* get_tt_entry(state_t* state, int index)
{
...
struct fsm_
{
	...
	/*If FSM needs to produce some output, the output data shall be stored in this buffer*/
...
void fsm_register_input_matching_fn_cb(fsm_t* fsm, input_matching_fn input_matching_fn_cb);
void set_fsm_initial_state(fsm_t* fsm, state_t* state);
void set_fsm_input_buffer_size(fsm_t* fsm, unsigned int size);
void set_fsm_default_input_fn(fsm_t* fsm, input_fn default_input_fn);
...
void register_input_matching_tt_entry_cb(tt_entry* tt_entry, input_matching_fn input_matching_fn_cb);
#endif 
